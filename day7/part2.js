...
const freeSpace = 70000000 - files["/"].size;
const spaceWeNeedToFree = 30000000 - freeSpace;
let closestDirectorySize = 70000000; //start with a huge number

const recursiveFolderCrawler = (folder) => {
  if (folder.size > spaceWeNeedToFree && folder.size < closestDirectorySize) {
    closestDirectorySize = folder.size;
  }
...
console.log(closestDirectorySize);
